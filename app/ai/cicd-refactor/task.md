...
- Current workflow files are **`.github/workflows/ci-pipeline.yml`, `release-push.yml`, `main-push.yml`**.
  ...
  three old files are deleted (Step 10). Do not leave both sets in place (the old ones still trigger).
...
Staging deploy: build + push ghcr.io image, deploy Next.js to Vercel preview.
...
Production deploy: same build/push structure as release, but Vercel **production** and Render deploy hook.
Tag format is `prod-v1.0.<timestamp>`.
...
### Step 9: Add the `deploy` job to `.github/workflows/main.yml`

**What**: Add a `deploy` job to `.github/workflows/main.yml` that triggers the Render deploy hook. No Aiven SQL init — the database is initialized manually on first
setup and must not be re-run on every deploy.
...
- Do NOT run any SQL or connect to Aiven.

**Done When**: `.github/workflows/main.yml` has a `deploy` job with `needs: push` and
`if: github.event_name == 'push'` that deploys to Vercel production with `--prod` flag, triggers the
Render deploy hook via curl, and has no Aiven or SQL steps.

---

## Phase 4 — Cleanup

### Step 11: Delete old workflow files

**What**: Remove the three legacy workflow files so they no longer trigger.

**How**: Delete `.github/workflows/ci-pipeline.yml`, `.github/workflows/release-push.yml`,
`.github/workflows/main-push.yml`.

**Done When**: Only `ci.yml`, `release.yml`, `main.yml` exist under `.github/workflows/`.

---

### Step 12: Final verification [HUMAN REVIEW]
...
- No hardcoded secrets/passwords: no `-proot`, no Docker Hub creds; ghcr.io uses `GITHUB_TOKEN`; all
  Vercel/Render values come from secrets.
- Staging (`release.yml`) deploy: Vercel preview only, no Render, no Aiven.
- Production (`main.yml`) deploy: Vercel production + Render deploy hook, no Aiven.
...
`RENDER_DEPLOY_HOOK_PRODUCTION`) and that the Vercel project, Render service, and Aiven database actually
exist — these cannot be verified from the repo alone and require human judgment before the workflows can
...
- Restructuring `server/`, `app/` source, or `docker/` beyond the single `docker-compose.yml`
  `MYSQL_ROOT_PASSWORD` env line (Step 2).
...
- Aiven database initialization (handled manually on first setup, not via CI/CD).
...
- `docker-compose.yml`, `app/package.json`
